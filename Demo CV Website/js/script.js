...
elm_signupform.addEventListener('submit', function(event){
    ...
    if ((newAccount.pswsignup) == (newAccount.psw1signup) ){
        ...
        document.getElementById("confirm-signup").innerHTML="Hai mật khẩu không trùng nhau"
    }
});
...
elm_form.addEventListener('submit', function(event){
    ...
    if(flag === 0){
        document.getElementById("confirm").innerHTML="Bạn chưa tạo tài khoản, xin bạn vô phần sign up ạ!"
    }else if(flag === 1){
        document.getElementById("confirm").innerHTML="Sai mật khẩu"
    }else{
        modal1.click();
        updateUsername(username);
    }
